...
func _process(delta):
	...
	if sensor==1 and Input.is_action_just_pressed("ui_down"):
			#anim.play("Good")
		...
		if dist > 52*0.3:
			...
			#print(str(dist) + " miss!, flecha abajo")
			anim.play("Bad")
			Global.scr-=10
			Global.combo=0
	...
	if sensor==0 && Input.is_action_just_pressed("ui_down"):
			anim.play("Bad")
			$"/root/Global".grd = "MISS !!"
			#$Grade/Label.text = "MISS !!"
			#print(str(dist) + " miss!, flecha abajo")
...
func _on_Idle_Down_area_shape_entered(area_rid, area, area_shape_index, local_shape_index):
...
func _on_Idle_Down_area_shape_exited(area_rid, area, area_shape_index, local_shape_index):
	sensor=0
	Global.sensor_AD=0
