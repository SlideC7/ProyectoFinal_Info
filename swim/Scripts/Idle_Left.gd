...
func _process(delta):
	...
	if sensor==0 && Input.is_action_just_pressed("ui_left"):
			anim.play("Bad")
			$"/root/Global".grd = "MISS !!"
			#$Grade/Label.text = "MISS !!"
...
func _on_Idle_Left_area_shape_entered(area_rid, area, area_shape_index, local_shape_index):
...
func _on_Idle_Left_area_shape_exited(area_rid, area, area_shape_index, local_shape_index):
	sensor=0
	Global.sensor_AL=0
